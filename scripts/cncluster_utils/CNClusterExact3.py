...
class CNClusterExact:
  ...
  def check_outliers_all(self, outf2):
    ...
    for ii in range(self.clus_info.shape[0]):
      ...
      cn1['ncarriers_var']=self.clus_info.info_ncarriers.values[ii]
      dd.append(cn1)
    dd=pd.concat(dd)
    cn1=dd.merge(self.carriers,  on=['varid', 'id', 'comp'], how='left')
    cn1['carrier']=np.where(cn1['info_ncarriers'].isnull(), 'non', 'carrier')
    cn1_ag=cn1.groupby(['varid', 'carrier', 'clus_id', 'dist_clus_id', 'comp', 'chunkstart', 'chunkstop', 'ncarriers_var'])['cn'].agg({'cn_med': np.median, 'cn_mad':robust.stand_mad}).reset_index()
    print(str(cn1_ag))
    ag_carriers=cn1_ag.loc[cn1_ag.carrier=="carrier"][['varid', 'clus_id', 'dist_clus_id', 'comp', 'cn_med', 'chunkstart', 'chunkstop', 'ncarriers_var']].rename(columns={'cn_med':'carrier_med'})
    ag_non=cn1_ag.loc[cn1_ag.carrier=="non"]   
    ag=ag_non.merge(ag_carriers, on=['varid', 'clus_id', 'dist_clus_id', 'comp', 'chunkstart', 'chunkstop', 'ncarriers_var'])
    if ag.shape[0]>0:
      ...
      if ag1.shape[0]>0:
        for varid in ag1['varid'].values:
          ...
          if n_outlier<0.01*self.nsamp and cn2['cn_mad'].values[0]<0.25:
            rare=True
            print("rare variant")
      res=ag[['comp', 'clus_id', 'dist_clus_id', 'chunkstart', 'chunkstop', 'comp', 'carrier_med', 'cn_med', 'cn_mad', 'dist', 'n_outliers', 'ncarriers_var', 'ncarriers_var', 'ncarriers_var', 'ncarrier_var']].values
      res[:,5]=0
      if rare:
        return res
      fits.append(res)
