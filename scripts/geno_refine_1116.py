...
vcf_rec = namedtuple ('vcf_rec', 'var_id sample svtype AF GT CN AB AP RP AS RS ')
...
def recluster(df):
    ...
    df.loc[:,'AP']=pd.to_numeric(df.loc[:,'AP'])
    df.loc[:,'RP']=pd.to_numeric(df.loc[:,'RP'])
    df.loc[:,'AS']=pd.to_numeric(df.loc[:,'AS'])
    df.loc[:,'RS']=pd.to_numeric(df.loc[:,'RS'])
    df.loc[:, 'AB_P']=df.loc[:,'AP']/(df.loc[:,'AP']+df.loc[:,'RP'])
    df.loc[:, 'AB_S']=df.loc[:,'AS']/(df.loc[:,'AS']+df.loc[:,'RS'])
    #df.to_csv('./temp.csv')
...
def recluster_DEL(df):
    ...
    mu_0_pairs={1:np.array([0.03, 2]),  2:np.array([0.32, 1.11]), 3:np.array([0.88, 0.20])}
    mu_0_spl={1:np.array([0.03, 2]), 2:np.array([0.22, 1.11]), 3:np.array([0.82, 0.20])}
    ...
    gpd=df.loc[:, ['gtn', 'CN', 'AB', 'AB_S', 'AB_P']].groupby(['gtn'])
    covs=gpd[['AB','CN']].cov()
    mns=gpd[['AB', 'CN']].mean()
    mns_pairs=gpd[['AB_P', 'CN']].mean()
    mns_split=gpd[['AB_S', 'CN']].mean()
    cts=gpd.size()
    ...
    with warnings.catch_warnings():
        ...
        if (lin_fit.rsquared>0.5) and (-1*lin_fit.params[1]>0.5):
             ...
             if x_int<1:
                 ...
                 gpd=df.loc[:,['gt_adj', 'CN', 'AB', 'AB_P', 'AB_S']].groupby(['gt_adj'])
                 covs=gpd[['AB', 'CN']].cov()
                 mns=gpd[['AB', 'CN']].mean()
                 mns_pairs=gpd[['AB_P', 'CN']].mean()
                 mns_split=gpd[['AB_S', 'CN']].mean()
    ...
    mu_map_pairs={1: get_mu_map(1, cts, lambda_0, mu_0_pairs, mns_pairs),
            2: get_mu_map(2, cts, lambda_0, mu_0_pairs, mns_pairs),
            3: get_mu_map(3, cts, lambda_0, mu_0_pairs, mns_pairs)}
    mu_map_spl={1: get_mu_map(1, cts, lambda_0, mu_0_spl, mns_split),
            2: get_mu_map(2, cts, lambda_0, mu_0_spl, mns_split),
            3: get_mu_map(3, cts, lambda_0, mu_0_spl, mns_split)}
    ...
    df.loc[:, 'lld1_spl']=multivariate_normal.logpdf(df.loc[:, ['AB_S', 'CN']], mean=mu_map_spl[1], cov=sigma_map[1])
    df.loc[:, 'lld2_spl']=multivariate_normal.logpdf(df.loc[:, ['AB_S', 'CN']], mean=mu_map_spl[2], cov=sigma_map[2])
    df.loc[:, 'lld3_spl']=multivariate_normal.logpdf(df.loc[:, ['AB_S', 'CN']], mean=mu_map_spl[3], cov=sigma_map[3])    
    
    df.loc[:, 'lld1_pairs']=multivariate_normal.logpdf(df.loc[:, ['AB_P', 'CN']], mean=mu_map_pairs[1], cov=sigma_map[1])
    df.loc[:, 'lld2_pairs']=multivariate_normal.logpdf(df.loc[:, ['AB_P', 'CN']], mean=mu_map_pairs[2], cov=sigma_map[2])
    df.loc[:, 'lld3_pairs']=multivariate_normal.logpdf(df.loc[:, ['AB_P', 'CN']], mean=mu_map_pairs[3], cov=sigma_map[3])
    ...
    lld_code_spl={'lld1_spl':1, 'lld2_spl':2, 'lld3_spl':3}
    df.loc[:,'gt_new_spl']=df.loc[:, ['lld1_spl', 'lld2_spl', 'lld3_spl']].idxmax(1).map(lld_code_spl)
    df.loc[:, 'gq_spl']=df.loc[:, ['lld1_spl', 'lld2_spl', 'lld3_spl']].max(axis=1)-df.loc[:, ['lld1_spl', 'lld2_spl', 'lld3_spl']].median(axis=1)
    df.loc[:, 'med_gq_spl']=df.loc[:, 'gq_spl'].median()
    df.loc[:, 'q10_gq_spl']=df.loc[:, 'gq_spl'].quantile(0.1)

    lld_code_pairs={'lld1_pairs':1, 'lld2_pairs':2, 'lld3_pairs':3}
    df.loc[:,'gt_new_pairs']=df.loc[:, ['lld1_pairs', 'lld2_pairs', 'lld3_pairs']].idxmax(1).map(lld_code_pairs)
    df.loc[:, 'gq_pairs']=df.loc[:, ['lld1_pairs', 'lld2_pairs', 'lld3_pairs']].max(axis=1)-df.loc[:, ['lld1_pairs', 'lld2_pairs', 'lld3_pairs']].median(axis=1)
    df.loc[:, 'med_gq_pairs']=df.loc[:, 'gq_pairs'].median()
    df.loc[:, 'q10_gq_pairs']=df.loc[:, 'gq_pairs'].quantile(0.1)
...
def load_df(var, exclude, sex):
    ...
    for s in var.sample_list:
        ...
        test_set.append(vcf_rec(var.var_id, s, var.info['SVTYPE'], var.info['AF'],
             var.genotype(s).get_format('GT'),  cn , var.genotype(s).get_format('AB'), var.genotype(s).get_format('AP'), var.genotype(s).get_format('RP'),
             var.genotype(s).get_format('AS'), var.genotype(s).get_format('RS')))
