...
def cluster(comp, cn_comp, info_comp, carriers_comp):
  ...
  if info_comp.shape[0]==1:
    ...
    df2=cnvsub2.pivot_table(index=['varid', 'varstart', 'varstop', 'nsamp'], columns='id', values='cn').reset_index()
    if comp==398:
      df2.to_csv('df2.csv')
    ...
    cnvsub2=cnvsub2.merge(df2, on=['varid', 'varstart', 'varstop', 'nsamp'])
  return cnvsub2[['varid', 'id', 'cn', 'varstart', 'varstop','comp', 'nsamp', 'carrier', 'cluster', 'dist_cluster']]
...
for comp in component_pos.comp.unique():
  tabixit=cntab.fetch( component_pos.chr.values[comp], max(0, component_pos.varstart.values[comp]-10), min(component_pos.varstop.values[comp]+10, np.max(component_pos.varstop)))
  s = StringIO.StringIO("\n".join(tabixit))
  cn=pd.read_table(s,  names=['chr', 'varstart', 'varstop', 'id', 'cn1'])
  cn=cn.merge(info,  on=['chr', 'varstart', 'varstop'])[['id', 'cn1', 'chr', 'varstart', 'varstop', 'comp']].drop_duplicates()
  ...
  if cn_comp.shape[0]>0:
    ...
    for [clus, dist_clus] in region_summary[['cluster', 'dist_cluster']].drop_duplicates().values:
      ...
      clus=CNClusterExact2.CNClusterExact(clus_vars, cn_comp, carriers_comp)
