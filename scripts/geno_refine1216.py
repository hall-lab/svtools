...
def recluster(df):
    ...
    if tp in ['DEL', 'MEI']:
        ...
        recluster_DUP(df)
        df.loc[:,'GTR']=df.loc[:, 'gt_new_bi'].copy()
        df.loc[:,'GTR2']=df.loc[:, 'gt_new_multi'].copy()
    elif tp in ['INV']:
...
def recluster_DUP_multi(df):
    ...
    df['gq_multi']=gq
    df['gt_new_multi']=gt+1
    df['med_gq_multi']=np.percentile(gq, 50)
    df['q10_gq_multi']=np.percentile(gq, 20)
...
def recluster_DUP_bi(df):
    ...
    df['gq_bi']=gq
    df['gt_new_bi']=gt+1
    df['med_gq_bi']=np.percentile(gq, 50)
    df['q10_gq_bi']=np.percentile(gq, 20)                                                                                                                                                        
