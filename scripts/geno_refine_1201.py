...
class gauss_mix:
    ...
    def __init__(self, mns=None, covs=None, pp=None, nn=3):
        self.__mu=mns
        self.__sigma=covs
        self.__probs=pp
        self.__ncomp=nn
    ...
    def __str__(self):
        string=str(self.__ncomp)+"\t"+str(self.__mu)+"\n"+str(self.__sigma)+"\n"+str(self.__probs)+"\n"
        return string
...
def recluster_DEL(df):
    ...
    ngps=len(cts)
    sys.stderr.write(str(mns)+"\n")
    
    #sys.stderr.write(str(len(cts))+"\n")
    #exit(1)
    ...
    if ngps==3:
        ...
        gm=gauss_mix(mu_map, sigma_map, {1:0.34, 2:0.33, 3:0.33}, 3)
        ...
        gm=gauss_mix(mu_map, sigma_map, {1:0.5, 2:0.5}, 2)


    
    Estep(df, gm, ngps, True)
    Mstep(df, gm, ngps)
    old_ll=log_lik(df, gm, ngps, True)
...
def Estep(df1, gm1, ncomp, init=False):

    if init:
        df1.loc[:, 'p1']=0
        df1.loc[:, 'p2']=0
        df1.loc[:, 'p3']=0
        df1.ix[df1['gt_adj']==1, ['p1', 'p2', 'p3']]=[1,0,0]
        df1.ix[df1['gt_adj']==2, ['p1', 'p2', 'p3']]=[0,1,0]
        df1.ix[df1['gt_adj']==3, ['p1', 'p2', 'p3']]=[0,0,1]
...
def Estep_mod(df1, gm1, ncomp):
    ...
    #df1.ix[(df1.AB<0.05) && (df1.p1<0.99), 'p1']=0.99
    #df1.ix[(df1.AB<0.05) && (df1.p1<0.99), 'p2']=0.01
    df1.ix[(df1['AB']<0.05) & (df1['p1']<0.999999), ['p1', 'p2', 'p3']]=[0.999999, 0.000001, 0.0]
    df1.ix[(df1['AB']>0.1) & (df1['p1']>0.01), 'p1']=0.01
    df1.ix[(df1['AB']>0.1), 'p2']=1-(df1.ix[(df1['AB']>0.10), 'p1']+df1.ix[(df1['AB']>0.10), 'p3'])

    if ncomp==3:
        df1.ix[(df1['AB']>0.9) & (df1['p3']<0.99), ['p1', 'p2','p3']]=[0.0, 0.01, 0.99]

    return



def Mstep(df1, gm1, ncomp):
...
def log_lik(df1, gm1, ncomp, init=False):
    ...
    if ncomp==2:
        df1.loc[:, 'lld']=(df1.loc[:, 'lld1']*gm1.get_probs(1)+df1.loc[:, 'lld2']*gm1.get_probs(2)).apply(np.log)
    elif ncomp==3:
        df1.loc[:, 'lld']=(df1.loc[:, 'lld1']*gm1.get_probs(1)+df1.loc[:, 'lld2']*gm1.get_probs(2)+df1.loc[:, 'lld3']+gm1.get_probs(3)).apply(np.log)
    lld=df1.loc[:, 'lld'].sum()
    return lld

def log_lik_mod(df1, gm1, ncomp):
    df1.loc[:, 'lld1']=multivariate_normal.pdf(df1.loc[:, ['AB', 'CN']], mean=gm1.get_mu(1), cov=gm1.get_sigma(1), allow_singular=False)
    df1.loc[:, 'lld2']=multivariate_normal.pdf(df1.loc[:, ['AB', 'CN']], mean=gm1.get_mu(2), cov=gm1.get_sigma(2), allow_singular=False)
    if ncomp==3:
        df1.loc[:, 'lld3']=multivariate_normal.pdf(df1.loc[:, ['AB', 'CN']], mean=gm1.get_mu(3), cov=gm1.get_sigma(3), allow_singular=False)
