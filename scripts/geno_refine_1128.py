...
def recluster_DEL(df):
    ...
    gm=gauss_mix()
    gm.probs={1:0.34, 2:0.33, 3:0.33}
    gm.sigma=sigma_map
    gm.mu=mu_map
    sys.stderr.write(str(gm.probs)+"\n")
    sys.stderr.write(str(gm.mu)+"\n")
    sys.stderr.write(str(gm.sigma)+"\n")

    df.loc[:, 'lld1']=multivariate_normal.pdf(df.loc[:, ['AB', 'CN']], mean=mu_map[1], cov=sigma_map[1])
    df.loc[:, 'lld2']=multivariate_normal.pdf(df.loc[:, ['AB', 'CN']], mean=mu_map[2], cov=sigma_map[2])
    df.loc[:, 'lld3']=multivariate_normal.pdf(df.loc[:, ['AB', 'CN']], mean=mu_map[3], cov=sigma_map[3])
    denom=df.loc[:,'lld1']*gm.probs[1]+df.loc[:, 'lld2']*gm.probs[2]+df.loc[:, 'lld3']*gm.probs[3]
    df.loc[:, 'p1']=df.loc[:,'lld1']*gm.probs[1]/denom
    df.loc[:, 'p2']=df.loc[:, 'lld2']*gm.probs[2]/denom
    df.loc[:, 'p3']=df.loc[:, 'lld3']*gm.probs[3]/denom
    pp={1:df[['p1']].mean(), 2:df[['p2']].mean(), 3:df[['p3']].mean()}
    ss1=sws.DescrStatsW(df[ ['AB', 'CN']], weights=df.loc[:, 'p1'])
    ss2=sws.DescrStatsW(df[['AB', 'CN']], weights=df.loc[:, 'p2'])
    ss3=sws.DescrStatsW(df[['AB', 'CN']], weights=df.loc[:, 'p3'])
    #df.loc[:, 'm1']=ss1.mean
    #df.loc[:, 'm2']=ss2.mean
    #df.loc[:, 'm3']=ss3.mean
    ...
    #ab1a=ss.mean
    #sig1=ss.cov
    #sig1=df[ ['AB', 'CN']].apply(np.cov, weights=df.loc[:, 'p1'])
    #sys.stderr.write(str(ab1)+"\n")
    #exit(1)
    #ab2=np.average( df.loc[:, 'AB'], weights=df.loc[:, 'p2'])
    #ab3=np.average( df.loc[:, 'AB'], weights=df.loc[:, 'p3'])
    sys.stderr.write(str(ss1.mean)+"\t"+str(ss2.mean)+"\n"+str(ss3.mean)+"\n")
