...
class gauss_mix:
    ...
    def __init__(self, mns=None, sig=None,  pp=None, nn=-1, dim=-1):
        self.__mu=mns
        self.__sigma=sig
        self.__probs=pp
        self.__ncomp=nn
...
def recluster_DEL(df):
    ...
    if ngps==3:
        ...
        sigma_map={1: covs.loc[1,:],
                   2: covs.loc[2,:],
                   3: covs.loc[3,:]}
        gm=gauss_mix(mu_map, sigma_map, {1:0.34, 2:0.33, 3:0.33}, 3, 2)
        ...
        sigma_map={1: covs.loc[1,:],
                   2: covs.loc[2,:]}
        gm=gauss_mix(mu_map, sigma_map, {1:0.5, 2:0.5}, 2, 2)
...
def Estep(df1, gm1):
    sys.stderr.write("in estep\n")
    ncomp=gm1.get_ncomp()
    dim=gm1.get_dim()
    calc_likelihoods(df1, gm1)
    df1.to_csv('./df1.csv')
    ...
    if ncomp==3:
        df1.loc[:, 'p3']=df1.loc[:, 'lld3']*gm1.get_probs(3)/denom  #zero if ncomp!=3
    else:
        df1.loc[:, 'p3']=0
...
def Mstep(df1, gm1):
    ...
    if dim==2:
        ...
        ss1=sws.DescrStatsW(df1[ ['AB']], weights=df1.loc[:, 'p1'])
        ss2=sws.DescrStatsW(df1[['AB']], weights=df1.loc[:, 'p2'])
        gm1.set_sigma(1, ss1.std+eps)
        gm1.set_sigma(2, ss2.std+eps)
        if ncomp==3:
            ss3=sws.DescrStatsW(df1[['AB']], weights=df1.loc[:, 'p3'])
            gm1.set_sigma(3, ss3.std+eps)
